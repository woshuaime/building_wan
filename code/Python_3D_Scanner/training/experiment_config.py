...
def preflight(config: dict[str, Any], *, check_checkpoints: bool = False) -> dict[str, Any]:
    ...
    model_paths = None
    configured_model_paths = runtime.get("model_paths")
    if configured_model_paths is not None:
        if not isinstance(configured_model_paths, list) or not configured_model_paths:
            raise ConfigError("runtime.model_paths 必须是非空路径列表")
        model_paths = [resolve_path(str(path)) for path in configured_model_paths]
        for index, path in enumerate(model_paths, start=1):
            require_file(path, f"本地模型文件 #{index}")

    tokenizer_path = None
    if runtime.get("tokenizer_path"):
        tokenizer_path = resolve_path(str(runtime["tokenizer_path"]))
        require_directory(tokenizer_path, "本地tokenizer目录")
        require_file(tokenizer_path / "tokenizer.json", "tokenizer.json")
