"""Run memory-safe two-stage Wan LoRA training on one or more GPUs."""
...
def normalize_gpu_ids(runtime: dict) -> list[int]:
    ...
    if not gpu_ids or len(set(gpu_ids)) != len(gpu_ids):
        raise ConfigError("Training requires at least one unique GPU index.")
...
def build_train_command(config, resolved) -> list[str]:
    ...
    command = [
        ...
        str(len(gpu_ids)),
        "--backend",
        str(runtime.get("distributed_backend", "gloo")),
        "--dataset_base_path",
        str(resolved["cache_dir"]),
        "--dataset_repeat",
...
def write_manifest(config, config_path, resolved, commands):
    ...
    payload = {
        ...
        "gpus": gpu_ids,
        "distributed_backend": runtime.get("distributed_backend", "gloo"),
...
def main():
    ...
    if args.stage in ("all", "cache"):
        ...
        if cached and not args.allow_existing:
            raise ConfigError(
                f"Cache already contains {len(cached)} files; use --allow-existing."
            )
        cache_env = env.copy()
        cache_env["CUDA_VISIBLE_DEVICES"] = str(gpu_ids[0])
        cache_env["PYTHONUNBUFFERED"] = "1"
        _run(commands["cache"], resolved["diffsynth_root"], cache_env)

    cached = list(resolved["cache_dir"].rglob("*.pth"))
    if args.stage in ("all", "train"):
        if not cached:
            raise ConfigError(f"No cached samples found in {resolved['cache_dir']}.")
